...
app.config["JWT_TOKEN_LOCATION"] = [
    "headers"
]  # options: "cookies", "json", "query_string"
app.config["JWT_COOKIE_SECURE"] = (
    True  # only allow the cookies that contain your JWTs to be sent over https
)
...
cors = CORS(
    app, resources={r"/api/*": {"origins": whitelist}}, supports_credentials=True
...
class UserPost(Resource):
    ...
    def post(self):
        ...
        if (
            len(args["username"]) == 0
            or len(args["username"]) > 20
            or len(args["password"]) > 20
        ):
        ...
        UserModel.create_user(
            args["username"], salt, hashed_password, description, avatar
        )
...
class Comment(Resource):
    ...
    def delete(self, idComment):
        ...
        parser.add_argument(
            "idUser", type=int, required=True, help="userId cannot be blank."
        )
...
class CommentPost(Resource):
    ...
    def post(self):
        ...
        parser.add_argument(
            "comment", type=str, required=True, help="comment cannot be blank."
        )
        args = parser.parse_args()
...
api.add_resource(
    CommentListOfUser, "/api/comments/<int:idUser>"
)  # 之後把userid改成token，前端拿不到userid
...
if __name__ == "__main__":
    ...
    app.run(
        ...
        port=5000,
        debug=True,
        ssl_context=("backend/assets/certificate.crt", "backend/assets/privateKey.key"),
